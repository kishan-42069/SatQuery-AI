...
def extract_raster_metadata(filepath: Union[str, Path]) -> RasterMetadata:
    ...
    with rasterio.open(path_str) as src:
        ...
        # Calculate WGS84 Geographic Extent for Map UI
        wgs84_bounds = {}
        if src.crs:
            try:
                if src.crs.is_geographic and (crs_epsg == 4326 or "4326" in str(crs_str)):
                    ...
                    min_lon, min_lat, max_lon, max_lat = transform_bounds(
                        src.crs, CRS.from_epsg(4326), src.bounds.left, src.bounds.bottom, src.bounds.right, src.bounds.top
                    )
                    wgs84_bounds = {
                ...
                logger.warning(f"Could not reproject raster bounds to WGS84: {e}")
                wgs84_bounds = {
                    "min_lon": float(src.bounds.left),
                    "min_lat": float(src.bounds.bottom),
                    "max_lon": float(src.bounds.right),
            ...
            # Fallback when CRS is missing in raster
            logger.warning(f"Raster {filepath} has no spatial reference system (CRS missing).")
            wgs84_bounds = {
                "min_lon": float(src.bounds.left),
                "min_lat": float(src.bounds.bottom),
                "max_lon": float(src.bounds.right),
...
def read_raster_bands(filepath: Union[str, Path], max_dimension: int = 1024) -> Tuple[np.ndarray, Tuple[int, int]]:
    """
    Reads raster bands into a numpy array, downsampling if larger than max_dimension.
    Returns (array (bands, H, W), original_shape (orig_H, orig_W)).
    """
